...
5. Show the full proposed commit message to the user using `request_user_input`. Offer three options: `commit only`, `commit and push`, and `request changes`. If the user wants edits, revise and confirm again with `request_user_input`.
6. After approval, stage only the intended files, create the commit, and verify success with `git status`.
7. Push only if the user approved `commit and push`.
...
- Do not push automatically; require `request_user_input` approval first.
