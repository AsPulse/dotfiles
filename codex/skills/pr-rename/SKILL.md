...
1. Identify the target PR. If the user supplied a PR number, use it. Otherwise resolve the PR from the current branch. If no PR is found, ask the user which PR to edit using `request_user_input`.
...
6. Show the old vs new title and body to the user using `request_user_input`. Offer exactly three choices: update title and body, update title only, or update body only. Rely on the automatic free-form `Other` option for requested changes. If the user wants changes, revise and confirm again with `request_user_input`.
