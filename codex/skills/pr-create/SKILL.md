...
3. Check whether the current branch has unpushed commits. If pushing is required before the PR can be created, explain that clearly and ask for approval with `request_user_input` before pushing.
...
7. Show the proposed title and body to the user using `request_user_input`. Briefly state the convention you observed so the user can confirm the match. Offer options to approve as-is or request changes. If the user wants changes, revise and confirm again with `request_user_input`.
